    ...
    pub async fn connect(&self) -> Result<RoomService<Connected>, ConnectError> {
        ...
        match tokio_ws2::connect_async(url).await {
            Ok((stream, _)) => {
                ...
                let mut conn = RoomConnection::start(stream, auth, exception_repoter)
                    .await
                        ...
                        for data in packet.clone().get_datas() {
                            match data {
                                ...
                                Data::Popularity(popularity) => {
                                    process_packet_broadcastor
                                        .send(Event::PopularityUpdate { popularity })
                                        .unwrap_or_default();
                                }
                                Data::Deflate(s) => {
            ...
            Err(e) => Err(ConnectError::WsError(e.to_string())),
        }
    }
}
...
impl RoomConnection {
    async fn start(
        ws_stream: WsStream,
        auth: Auth,
        exception: mpsc::Sender<Exception>,
    ) -> Result<Self, RoomConnectionError> {
        use ws2::Message::*;
        ...
        let _auth_reply = match rx.next().await {
            ...
            other @ _ => {
                ...
                return Err(RoomConnectionError::FailToAuth {
                    msg: format!("{:?}", other),
                });
            }
        };
        let channel_buffer_size = 64;
        let (pack_outbound_tx, mut pack_outbound_rx) =
            mpsc::channel::<RawPacket>(channel_buffer_size);
        ...
        let send = async move {
            while let Some(p) = pack_outbound_rx.recv().await {
                ...
                match tx.send(Binary(bin)).await {
                    ...
                    Err(e) => {
                        repoter
                            .send(Exception::WsSendError(e.to_string()))
                            .await
                            .unwrap();
                    }
                }
        ...
        let recv = async move {
            while let Some(Ok(msg)) = rx.next().await {
                match msg {
                    ...
                    Close(f) => {
                        repoter
                            .send(Exception::WsDisconnected(format!("{:?}", f)))
                            .await
                            .unwrap();
                        break;
                    }
                    ...
                    msg @ _ => {
                        repoter
                            .send(Exception::UnexpectedMessage(msg))
                            .await
                            .unwrap();
                    }
                }
